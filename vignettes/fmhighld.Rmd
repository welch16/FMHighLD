...
author: "Rene Welch, Kwangmoon Park and Sunduz Keles"
package: FMHighLD
output: 
...
snp_data <- tibble::tibble(
  ...
  dplyr::mutate(
    ...
    skew) %>%
  dplyr::mutate(
    z = rnorm(nsnps, if_else(causal == 1, 0 +  1 * skew, 0.0), sd = .5))
...
true_model <- lm(z ~ 0 + skew, data = as.data.frame(true_data) %>%
  tibble::column_to_rownames("snp"))

true_data %<>%
  dplyr::mutate(res = residuals(true_model)[true_data$snp]^2)

# select_causals_single(snp_data, true_model, FMParam(max_iter = 20),
#   "ld_cluster", "skew") %>%
#   dplyr::inner_join(
#     true_data, by = "ld_cluster")

# true_model

# fmld_fit <- fmhighld_fit(
#   response, skew_mat, ld_clusters,
#   singletrait = TRUE, ncausal_mixt = 1,
#   formula = response ~ 0 + annot,
#   # init_coef_mean = true_model$coefficients,
#   skip_causal = FALSE, fm_param = FMParam(strategy = "none",
#     min_tol = 1e-3, max_iter = 100, annot_tol = 1e-2),
...
fmld_fit <- fmhighld_fit_em(
  ...
  # init_coef_mean = true_model$coefficients,
  skip_causal = FALSE, fm_param = FMParam(strategy = "none",
    min_tol = 1e-3, max_iter = 50, annot_tol = 1e-2),
  save_iter = FALSE, verbose = TRUE)

tibble::tibble(id = seq_along(fmld_fit$loglike),
  loglike = fmld_fit$loglike) %>%
  ggplot(aes(id, loglike)) + geom_line()



fmld_fit$all_models[[which.max(fmld_fit$loglike)]] %>%
  flexmix::relabel() %>%
...
snp_data %>%
  ...
  geom_abline(slope = pps[[1]][2, 1], intercept = pps[[1]][1, 1],
    colour = "magenta") +
  geom_abline(slope = 0, intercept = pps[[2]][1, 1], colour = "darkgreen",
    linetype = 2)
