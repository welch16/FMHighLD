...
tt <- tibble::tibble(id = seq_along(fmld_fit_em$loglike),
  loglike = fmld_fit_em$loglike,
  full = fmld_fit_em$full_loglike) %>%
  mutate(ratio = loglike / full, diff = loglike - full)
...
cowplot::plot_grid(
    ...
    tt %>%
      ...
      theme(axis.title.x = element_blank()), ncol = 1)


fmld_fit_em$all_models[[which.max(tt$diff)]] %>%
  flexmix::relabel() %>%
